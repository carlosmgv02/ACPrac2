             ...
             unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BpredCascade:
    ...
    for (int cnt = 0; cnt < l2size; cnt++)
    {
      pred->dirpred.twolev->config.two.l2table[cnt] = 4; //incialitzem a 4
    }

    for (int cnt = 0; cnt < bimod_size; cnt++)
    {
      pred->dirpred.bimod->config.bimod.table[cnt] = 3; //incialitzem a '11'= 3
    }
					 ...
					 * used on mispredict recovery */
{
  ...
  case BpredCascade:
    if ((MD_OP_FLAGS(op) & (F_CTRL | F_UNCOND)) != (F_CTRL | F_UNCOND))
    {
      ...
      bimod = bpred_dir_lookup(pred->dirpred.bimod, baddr);
      twolev = bpred_dir_lookup(pred->dirpred.twolev, baddr);
      dir_update_ptr->dir.bimod = *bimod;
      dir_update_ptr->dir.twolev = *twolev;
                  ...
                  struct bpred_update_t *dir_update_ptr) /* pred state pointer */
{
  ...
  {
    if (dir_update_ptr->dir.meta || bpred_dir_lookup(pred->dirpred.twolev, baddr) == dir_update_ptr->pdir1)
      pred->used_2lev++;
    else
      pred->used_bimod++;
  }
  ...
  if (dir_update_ptr->pdir2 != NULL && *dir_update_ptr->pdir2 != 4)
  {
  ...
  else if (!!taken != !!pred_taken && dir_update_ptr->pdir1 != NULL)
  {
